...
type FilesJsonType = Record<
  string,
  { files: { name: string; content: string; path: string; language: string; type: string }[] }
>;
...
export default function Home({ defaultDemoId }: HomePageProps = {}) {
  ...
  const [selectedFramework, setSelectedFramework] = useState<string>(
    (params?.agent as string) || (AGENT_TYPE == "general" ? "crewai" : AGENT_TYPE),
  );
  const [toDelete, setToDelete] = useState<boolean>(false);
  // Filter demos based on the selected framework OR if they have an iframeUrl or special ID
  const filteredDemos = config.filter(
    (d) => d.id === "research-canvas" || d.iframeUrl || d.id.startsWith(`${selectedFramework}_`),
  );
  ...
  const demoFiles = (
    selectedDemoId && (filesJson as FilesJsonType)[selectedDemoId]
      ? (filesJson as FilesJsonType)[selectedDemoId].files
      : []
  ).map((file) => ({
    // Ensure the mapped type matches FileEntry
    ...file,
  ...
  const handleFileSelect = useCallback(
    (filePath: string | null): void => {
      ...
      if (filePath && selectedDemoId) {
        const fileData = (filesJson as FilesJsonType)[selectedDemoId]?.files.find(
          (f) => f.path === filePath,
        );
  ...
  const handleTabChange = useCallback(
    (value: string): void => {
      ...
      if (value === "code" && demoFiles.length > 0) {
        const agentPyFile = demoFiles.find((file: FileEntry) => file.name === "agent.py");
        if (agentPyFile) {
                          ...
                          <div className="h-full">
                            <CodeEditor
                              file={{
                                ...
                                language:
                                  (selectedDemoId &&
                                    (filesJson as FilesJsonType)[selectedDemoId]?.files.find(
                                      (f: any) => f.path === selectedFilePath,
                                    )?.language) ||
                                  "plaintext",
                              }}
                            />
